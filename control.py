...
class Seguimiento:
    ...
    def actualizar(self,inc=1,x=0,y=0):
        tmp = open("temp", "r+")
        ...
        if obj["iniciado"]:
            ...
            if obj["procesados"] % self.divisor == 0:
                ...
                text = f"." * (obj["procesados"]//self.divisor) + " " * ((obj["total"] - obj["procesados"])//self.divisor) + f"{obj["porcentaje"]}%\n"
...
class Imp:
    ...
    def text(self,text):
        ...
        t = dt.datetime.today()
        d = str(t)[5:-5]
        text = f"|{d[3:5]} de {mes[int(d[0:2])-1] } {d[6:]}|  {str(text)}"
